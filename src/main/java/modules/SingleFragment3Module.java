...
public class SingleFragment3Module extends Module {
	...
	public SingleFragment3Module(GeneralModel generalModel, Fragment fragment, VariableStrategy variableStrategy,
	...
	public void buildVariables() {
		...
		for (int i = 0; i < fragment.getNbNodes(); i++) {
			...
			if (fragment.getLabel(i) == 2)
				coronenoidCorrespondances[i] = generalModel.getProblem().intVar("coronenoid_correspondances_" + i,
						domain);

			else
				coronenoidCorrespondances[i] = generalModel.getProblem().intVar("coronenoid_correspondances_" + i,
						domainWithOutterHexagons);
