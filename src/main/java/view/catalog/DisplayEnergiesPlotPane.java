...
public class DisplayEnergiesPlotPane extends GridPane {
	...
	private void initialize() {
		...
		saveDataButton.setOnAction(e -> {
			...
			File file = directoryChooser.showDialog(parent.getApp().getStage());
		...
		exportButton.setOnAction(e -> {
			...
			fileChooser.setInitialFileName(selectedPlotPane.getResult().getPAHClass().getTitle() + ".png");
		...
		exportAllButton.setOnAction(e -> {
			...
			if (file != null) {
				...
				for (int i = 0; i < panes.size(); i++) {
					...
					if (unix)
						plotFile = new File(
								directoryPath + "/" + selectedPlotPane.getResult().getPAHClass().getTitle() + ".png");
					else
						plotFile = new File(
								directoryPath + "\\" + selectedPlotPane.getResult().getPAHClass().getTitle() + ".png");
	...
	private void exportSpectres(File file) throws IOException {
		...
		for (ComputedPlotPane pane : panes) {
			ResultSpectrums result = pane.getResult();
	...
	private void exportMoleculesInformations(File file) throws IOException {
		...
		for (ComputedPlotPane pane : panes) {
			...
			ResultSpectrums resultSpectrum = pane.getResult();

			writer.write(resultSpectrum.getPAHClass().getTitle() + "\n");
